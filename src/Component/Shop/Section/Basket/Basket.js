...
export default function Basket({productName, cardClick}){
            ...
            <div>
                {aray.map((elem, index)=>{
                    return <div key={index} className="basketCard">
                        ...
                        <Link to={`/product:${productName}`}><h4 id={elem.id} onClick={cardClick}>{elem.model}</h4></Link>
                            {elem.price? <span>{`${elem.price.toLocaleString()}AMD`}</span>:<span>{elem.Price.toLocaleString()}</span>}
                        </div>
                        ...
                        <div className="totalPrice">{`${elem.cardTotalPrice.toLocaleString()}AMD`}</div>
                        <div className="Remove">
                            <button onClick={removeProduct} id={elem.id}>Remove</button>
                        </div>
                    </div>
                })}
                <div>{`${totalPrice.toLocaleString()}AMD`}</div>
            </div>
        </section>
    )
}