...
export default function Section({cardClick, productName, selectLanguage}){
...
const filterRemove=(filter)=>{
        const llll =  phoneCategory.filter((elem)=>filter !== elem.RAM)
        setff(false)
        if(filter == "3GB"){
    ...
    return(
        ...
        <section>
            <div className="categoryPage">
                <Filter setFilter={setFilter} setff={setff} selectLanguage={selectLanguage}></Filter>
             ...
             {filterr.map((elem, index)=>{  
                return <div className="card" id={elem.id} key={index}>
                   ...
                   <div className="cardProductNamePhones">
                    ...
                    <p>{`${elem.price.toLocaleString()}AMD`}</p>
                    <button onClick={addBasket} id={elem.id} className="ddd">Buy</button>
